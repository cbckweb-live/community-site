  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="space-y-5 mb-10 bg-white shadow-md rounded-2xl p-6">
        ...
        <div>
          <p className="text-sm text-[#231F1E]/60 mb-2">Attach media (optional)</p>
          <div className="flex gap-4 mb-3">
            {(["none", "photo", "pdf"] as const).map((type) => (
              <label key={type} className="flex items-center gap-2 text-sm capitalize">
                <input type="radio" checked={mediaType === type} onChange={() => { setMediaType(type); setMediaFile(null); }} />
                {type}
              </label>
            ))}
          </div>
        ...
        <div className="flex gap-3">
          ...
          {editingId && (
            <button type="button" onClick={() => { setForm(emptyPost()); setEditingId(null); setMediaType("none"); }} className="text-sm text-[#231F1E]/50 hover:underline">Cancel</button>
          )}
        </div>
      </form>
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-white shadow-sm rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-[#231F1E]/50">{post.category} · {new Date(post.created_at).toLocaleDateString()}</p>
            </div>
